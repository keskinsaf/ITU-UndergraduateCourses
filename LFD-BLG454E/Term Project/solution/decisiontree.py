...
def calculateAccur( tdata,y ):
	pr = clf.predict_proba(tdata)
	pr = pr[:,1]
	...
	for i in range(pr.shape[0]):
	    if pr[i] == float(Y[i]):
	        acc_ctr += 1
	    abs_error += abs( pr[i] - Y[i] )
	    # print( abs( pr[i] - Y[i] ) )
	print("Absolute error: %f" % abs_error )
	print("Accuracy: %f percent" % (100 * acc_ctr / pr.shape[0] ) )
...
prediction.to_csv("lastprediction.txt", header = ['orderItemID,returnShipment'], index=True)

calculateAccur( traindata, Y )